...
def get_current_teacher(token: str = Depends(oauth_scheme), db: Session = Depends(database.get_db)):
    ...
    teacher = db.query(models.Teacher).filter(models.Teacher.id == token.id).first()
    return teacher
